		...
		<div class="col-xs-12 col-sm-8 center-block">
			<div class="panel panel-default login-panel">
				...
				<div class="panel-body">
					...
					@endif
					<form method="POST" action="{{ URL::to('administrador/nuevo-horario/enviar') }}">
						<div class="col-xs-12">
							...
							<div class="input-field">
								<label>Curso (*)</label>
							  	<select name="course" class="form-control validate-input course">
							  		<option value="">Seleccione una opción...</option>
							  		@if(Input::old('course'))
							  			<option value="{{ Input::old('course') }}" selected></option>
							  		@endif
							...
							<div class="input-field">
								<label>Dia (*)</label>
							  	<select name="day" class="form-control validate-input">
							  		<option value="">Seleccione una opción...</option>
							  		@foreach($days as $d)
							  			<option value="{{ $d->id }}" @if(Input::old('day') && Input::old('day') == $d->id) selected @endif>{{ $d->name }}</option>
							  		@endforeach
							  	</select>
							</div>
						</div>
						<div class="col-xs-12 col-md-6">
							<div class="input-field">
								<label>Inicio (*)</label>
							  	<input type="text" name="start_at" class="form-control validate-input" placeholder="Inicio" value="{{ Input::old('start_at') }}">
							</div>
						</div>
						<div class="col-xs-12 col-md-6">
							<div class="input-field">
								<label>Fin (*)</label>
							  	<input type="text" name="end_at" class="form-control validate-input" placeholder="Fin" value="{{ Input::old('end_at') }}">
							</div>
						</div>
						<div class="col-xs-12">
							<div class="input-field">
								<label>Sección / Grupo (*)</label>
							  	<input type="text" name="group" class="form-control validate-input" placeholder="Sección / Grupo" value="{{ Input::old('group') }}">
							</div>
							<div class="input-field">
								<label>Salón (*)</label>
							  	<input type="text" name="location" class="form-control validate-input" placeholder="Salón" value="{{ Input::old('location') }}">
							</div>
							<div class="input-field">
								<label>¿Activo?</label>
								<input type="checkbox" name="active" value="1" class="switch" @if(Input::old('active')) checked @endif>
							</div>
						</div>
						{{ Form::token() }}
					</form>
